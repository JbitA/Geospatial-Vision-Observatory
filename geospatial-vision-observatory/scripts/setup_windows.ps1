...
function Find-Python312 {
    ...
    if ($null -ne $PyLauncher) {
        ...
        if ($LASTEXITCODE -eq 0) {
            return [PSCustomObject]@{ Exe = $PyLauncher.Source; Args = @("-3.12") }
        }
    }
    ...
    throw @"
Python 3.12 is required for the reproducible Windows environment.
...
If this still occurs, install Python 3.12 from python.org (or winget Python.Python.3.12),
