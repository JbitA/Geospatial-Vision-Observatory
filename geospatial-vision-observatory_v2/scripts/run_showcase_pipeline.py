...
from geo_vision.experiment import record_holdout_exposure  # noqa: E402
...
def record_external_evaluation_exposure(
    evaluation_path: Path = Path("reports/landcover/evaluation.json"),
    ledger_path: Path = Path("reports/experiment-state/holdout-exposure.jsonl"),
) -> None:
    """Record that the final selected experiment has inspected external holdout pixels."""

    evaluation = json.loads(evaluation_path.read_text(encoding="utf-8"))
    if not isinstance(evaluation, dict) or "external_test" not in evaluation:
        raise RuntimeError("external evaluation evidence is missing")
    experiment_signature = evaluation.get("experiment_signature")
    data_signature = evaluation.get("dataset_signature")
    if not isinstance(experiment_signature, str) or not isinstance(data_signature, str):
        raise RuntimeError("external evaluation signatures are missing")

    baseline_raw = json.loads(Path("config/aois-baseline.geojson").read_text(encoding="utf-8"))
    if not isinstance(baseline_raw, dict):
        raise RuntimeError("baseline AOI document must be a JSON object")
    by_id = {aoi.aoi_id: aoi for aoi in load_aoi_document(baseline_raw)}
    try:
        external = [by_id[aoi_id] for aoi_id in SHOWCASE_SPLIT.external_test]
    except KeyError as error:
        raise RuntimeError(f"external AOI is missing from baseline configuration: {error}") from error
    record_holdout_exposure(
        ledger_path,
        experiment_signature=experiment_signature,
        dataset_signature=data_signature,
        aois=external,
        purpose="final external evaluation for validation-selected showcase experiment",
    )
...
def main() -> None:
    ...
    if args.quick:
        ...
        if args.force_train:
            train.append("--force")
        run(train)
        record_external_evaluation_exposure()
