...
def spatial_block_bootstrap_macro_iou_ci(
    block_confusions: Sequence[npt.NDArray[np.int64]],
    *,
    iterations: int = 1_000,
    seed: int = 20260823,
    alpha: float = 0.05,
) -> tuple[float, float]:
    """Bootstrap macro IoU by spatial blocks rather than individual overlapping patches."""

    if not block_confusions:
        return math.nan, math.nan
    rng = np.random.default_rng(seed)
    stack = np.stack(block_confusions)
    block_count = len(block_confusions)
    values: list[float] = []
    for _ in range(iterations):
